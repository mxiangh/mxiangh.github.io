...
$E= \lbrace (u, v) |u∈V, v∈V \rbrace$，用$\vert E \vert$表示图G中边的条数。
...
def cal_pairwise_dist(x):
    ...
    (a-b)^2 = a^2 + b^2 - 2*a*b
    '''
    sum_x = np.sum(np.square(x), 1)  # 求每个样本的L2范数平方
...
def IsoMap(X, n=2, n_neighbors=30):
    ...
    data_n = mds(D_floyd, n_components=n)
    return data_n
...
def IsoMap(X, n=2, n_neighbors=30):
    ...
    # 应用MDS降维
    data_n = mds(D_dijkstra, n_components=n)
    return data_n
...
# 应用sklearn的ISOMAP降维，默然Dijkstra算法
# n_neighbors: 近邻数量，n_components: 降维后的维度
