...
![](/images/ISOMAP/two.png)

![](/images/ISOMAP/three.png)

![](/images/ISOMAP/four.png)
...
笔者实现代码时，发现手写和sklearn不一样，经过摸索，发现是因为在输入MDS模型里面的距离没有平方处理，而是将原始数据计算欧氏距离平方后求最短路径，将最短路径直接放入MDS，发现降维后的图形与LLE相似，后将原始数据只计算欧氏距离，求最短路径，最短路径平方后放入MDS，得到和sklearn相同结果。
至于为什么欧氏距离平方后求最短路径直接放入MDS，会得到LLE类似结果，这个问题有待考量。
