...
$$ \sum_{i \ne j} (\left\|z_i-z_j\right\|-d_{ij})^2 $$

其中，$\left\|z_i-z_j\right\|$表示低维样本两点之间欧氏距离，$ d_{ij} $表示高维空间两点距离，高维空间距离可选任意距离，但是要满足距离度量条件。
...
$$ \frac{\sum_{i \ne j} (\left\|z_i-z_j\right\|-d_{ij})^2}{\sum_{i \ne j} d_{ij}^2 } $$
...
$$ \left\|z_i^{'} - z_j^{'} \right\| = \left\|(z_i - z_0) - (z_j - z_0) \right\| = \left\|z_i - z_j \right\|  $$

2. 距离矩阵与内积矩阵转化（需要较好的线性代数基础）

假设中心化后的数据矩阵 $ X \in \mathbb{R}^{n \times D} $，每行一个样本，其内积矩阵$ B = X X^T$，距离矩阵D满足 $D_{ij}=\left\|x_i-x_j\right\|$。

（1）距离平方展开

$$ D_{ij}^2 = \left\|x_i-x_j\right\|^2 = \left\|x_i\right\|^2 + \left\|x_j\right\|^2 - 2 x_i^T x_j $$

（2）矩阵形式表示：

定义：

$$ s = \lbrack \left\|x_i\right\|^2, \left\|x_i\right\|^2, \dots , \left\|x_i\right\|^2 \rbrack^T ，\mathbf{1}=  \lbrack1, 1,\dots,1 \rbrack^T $$
...
&= -\frac{1}{2} ((\left\|x_i\right\|^2 + \left\|x_j\right\|^2 - 2 x_i^T x_j) - \frac{1}{n} \sum_{k=1}^n (\left\|x_i\right\|^2 + \left\|x_k\right\|^2 - 2 x_i^T x_k) - \frac{1}{n} \sum_{k=1}^n (\left\|x_k\right\|^2 + \left\|x_j\right\|^2 - 2 x_k^T x_j) + \frac{1}{n^2} \sum_{k,l=1}^n (\left\|x_k\right\|^2 + \left\|x_l\right\|^2 - 2 x_k^T x_l)) \\
&= -\frac{1}{2} ((\left\|x_i\right\|^2 + \left\|x_j\right\|^2 - 2 x_i^T x_j) - (\left\|x_i\right\|^2 + \frac{1}{n} \sum_{k=1}^n \left\|x_k\right\|^2) - (\left\|x_j\right\|^2 + \frac{1}{n} \sum_{k=1}^n \left\|x_k\right\|^2) + \frac{2}{n} \sum_{k,l=1}^n \left\|x_k\right\|^2) \\
&= x_i^T x_j
\end{aligned}$$
...
定义：对于任意一个实矩阵 $A_{m\times n}$（m行n列），奇异值分解是将其分解为三个矩阵的乘积形式，即：$A=U \Sigma V^T $。
...
原理：从线性变换的角度来看，矩阵A可以看作是一个从n维空间到m维空间的线性变换。奇异值分解就是将这个线性变换分解为三个连续的简单变换：
...
$$ \underset{Z}{min} \sum_{i < j} (\left\|z_i-z_j\right\|-d_{ij})^2 $$

- 非度量MDS：仅保持距离排序关系，适用于定性数据

$$ \underset{Z}{min} \sum_{i < j} (f(D_{ij}) - \left\|z_i-z_j\right\|)^2 $$
...
# 应用MDS降维到2D
X_mds = mds(X_scaled, n_components=2)
...
# 使用sklearn的MDS进行降维
mds = MDS(n_components=2, random_state=42, normalized_stress='auto')  # 设置随机状态以确保结果可复现
X_mds = mds.fit_transform(X_scaled)  # 拟合模型并转换数据

# 可视化结果
plt.figure(figsize=(8, 6))
for i, class_name in enumerate(class_names):
    mask = y == i
    plt.scatter(X_mds[mask, 0], X_mds[mask, 1],
                label=class_name,
                edgecolor='k',
                s=70)
