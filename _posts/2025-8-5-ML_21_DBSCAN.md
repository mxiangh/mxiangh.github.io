...
对$ \mathbf{x}_j \in D $，其$\epsilon$-邻域包含样本集D在与$ \mathbf{x}_j $的距离不大于$ \epsilon $的样本，即；

人话：以样本$\mathbf{x}_j$为圆心，$\epsilon$为半径画圆，圆内和圆上的其他样本都是样本$\mathbf{x}_j$的$\epsilon$-邻域。

- 核心对象（core object）

若$ \mathbf{x}_j $的$ \epsilon $-邻域至少包含MinPts个样本，即，则$\mathbf{x}_j$是一个核心对象。MinPts是一个超参数；
...
对$ \mathbf{x}_j $与$ \mathbf{x}_i $，若存在样本序列，其中且$ p_{i+1} $由$ p_i $密度直达，则称$ \mathbf{x}_j $由$ \mathbf{x}_i $密度可达；
