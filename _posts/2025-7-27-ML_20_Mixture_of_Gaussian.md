...
（4）全概率公式：$P(A)=\sum_{i = 1}^{n}P(A|B_i)P(B_i)$，把事件 A 发生的概率，通过样本空间的划分，分解为在不同条件 $B_i$ 下 A 发生的概率的加权和，权重就是条件 $B_i$ 本身发生的概率。
...
# 假设西瓜数据集4.0如下（示例数据，实际数据请替换为真实数据）
# 数据格式：密度，含糖率
data = np.array([
...
# 预测每个样本的聚类标签
labels = gmm.predict(watermelon_data)
