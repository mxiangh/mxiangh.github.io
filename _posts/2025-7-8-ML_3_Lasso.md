...
$$ E_{\hat{w}} = min (\mathbf{y}-\mathbf{X} \hat{w})^T (\mathbf{y}-\mathbf{X} \hat{w}) + \lambda \vert \vert \hat{w} \vert \vert $$ 

其中，$ \lambda $是一个自定义的超参数，$ \vert \vert \hat{w} \vert \vert $ 是$ L_1 $范数。

##### 2.范数

强化过后的距离概念，可以把范数理解为距离。设向量$ x=(x_1,x_2,\dots,x_n)^T $，则$ L_1 $范数为

$$ \vert \vert x\vert \vert_1 = \vert x_1 \vert +\vert x_2\vert +\dots+\vert x_n\vert  $$
