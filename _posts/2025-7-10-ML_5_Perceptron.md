...
超平面$H$是从$n$维空间到$n-1$维空间的一个映射子空间，它有一个$ n$ 维向量和一个实数定义。因为是子空间，所以超平面一定过原点。

通俗来说，只需要知道超平面在二维是一条直线，在三维是一个平面，依次类推即可。

##### 2.感知机

定义：假设输入空间（特征空间）是 $ \mathcal{X} \subseteq R^{n} $ ，输出空间是 $ \mathcal{Y}=\lbrace +1,-1 \rbrace $ 。输入 $ x \in \mathcal{X} $ 表示实例的特征向量，对应于输入空间（特征空间）的点；输出 $ y \in \mathcal{Y} $ 表示实例的类别。由输入空间到输出空间的如下函数
...
由于感知机目标函数不可导，无法直接求解析解，所以只能通过迭代优化求取参数。通常使用随机梯度下降法（SGD）优化算法。 （参考Tips1的梯度介绍）

##### 5.缺点

感知机是一个线性模型，最大的缺点是不能表示异或函数，这为后续神经网络中隐藏层的出现做铺垫。
...
代码注解：第一段手写没有标准化，训练时出现误差，第二段代码使用标准化（通常均值为 0，标准差为 1 ），训练和测试都是百分百，第三段使用sklearn实现。
